...
**Structure Overview:**
1. **User Management:** Represents actors (`GuestUser`, `RegisteredUser`) interacting with the system to browse games and checkout.
2. **Game Catalog (Singleton Pattern):** `GameCatalog` is a single, centralized instance managing all `Game` entities, which in turn contain specific denomination `Package`s.
3. **Order Construction (Builder Pattern):** The `TopUpOrder` contains all transaction details. Due to its complexity, it is constructed step-by-step using the `OrderBuilder`, which sets player info, packages, and applies `PromoCode`s.
4. **Order State (State Pattern):** The lifecycle of a `TopUpOrder` is governed by the `OrderState` interface, delegating behaviors (pay, deliver) to concrete states (`PendingState`, `PaidState`, `CompletedState`, `FailedState`) to ensure valid transitions.
5. **Payment Processing (Abstract Factory & Strategy Patterns):** The `PaymentProcessor` utilizes the Strategy pattern to execute payments dynamically (e.g., `ABAPayStrategy`). The Abstract Factory (`PaymentGatewayFactory`) ensures the correct families of payment handlers and receipt generators are created for different gateways.
6. **Publisher Integration (Facade Pattern):** The `PublisherFacade` provides a simplified, unified interface for the system to validate players and deliver currency, hiding the complexities of the external `GamePublisherAPI`.
...
**Presentation Layer**
- **Tech:** React (bootstrapped with Vite) + TypeScript.
- **Role:** The UniPin Web Portal React App. Vite provides near-instant hot module replacement (HMR) during development and compiles down to highly optimized static assets (HTML/JS/CSS) for production deployment.

**Business Logic & Integration Layers**
- **Tech:** Node.js + Express.js (JavaScript).
- **Role:** The Express backend is structured to handle orders, promotions, and payment processing routes. It serves as the core API that processes incoming requests from the frontend portal.

**Data Access Layer**
- **Tech:** Mock Data / In-Memory (JSON).
- **Role:** In line with the project's scope for a Mock Topup Demo, the data access layer relies on simplified local mock data for game catalogs and transactions rather than an external database or caching infrastructure.
